...
import {
  ChevronDown, Menu, X, Factory, CircuitBoard,
  Users, Building2, Store, Blocks, Globe, SmartphoneNfc  , NotebookText  , Container , Warehouse  , Cog , ShieldCheck  , BadgeCheck 
} from 'lucide-react';
...
const Navbar = () => {
  ...
  const navLinks = [
    ...
    },
    {
      name: "Markets",
      items: [
        { name: "Market Overview", icon: Store },
        { name: "Industrial & Automation", href: "/markets/industrial-automation", icon: Factory },
        { name: "Smart Living & Lifestyle", href: "/markets/smart-living", icon: SmartphoneNfc  },
      ]
    },
    {
      name: "Services",
      items: [
        { name: "Integration Services", icon: Blocks },
        {name : "Research and Development" , href  : "/services/research-and-development", icon : NotebookText  },
        { name: "Quality Assurance", href: "/services/quality-assurance", icon: BadgeCheck  },
        { name: "Supply Chain Management", href: "/services/supply-chain-management", icon: Container  },
        { name: "Fullfillment & Logistics", href: "/services/fullfillment-logistics", icon: Warehouse  },
        { name: "Test Development", href: "/services/test-development", icon: ShieldCheck  },
        { name: "PCB Manufacturing", href: "/services/pcb-manufacturing", icon: CircuitBoard },
        { name: "Machining & Tooling", href: "/services/machining-tooling", icon: Cog  },
      ]
    },
        ...
        <div className="relative" onMouseLeave={() => setActiveDropdown(null)}>
          <div className="flex items-center justify-between">
            ...
            <nav className="hidden lg:flex items-center space-x-2 relative z-50">
              {navLinks.map(item => (
                <div key={item.name} className="relative" onMouseEnter={() => item.items && setActiveDropdown(item.name)}>
                  {item.items ? (
                    <button className={cn(
                    ...
                    <Link
                      to={item.href}
                      className={cn(
                        'text-sm font-medium px-4 py-2 uppercase hover:text-blue-600',
                        shouldUseDarkText ? 'text-black' : 'text-white'
                      )}
                    >
                      {item.name}
          ...
          {activeDropdown && (
            <div className={cn(
              ...
              <div className="max-w-7xl mx-auto px-6 py-6 grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-6">
                {navLinks.find(item => item.name === activeDropdown)?.items?.map((subItem) => {
                  const Icon = subItem.icon;
                  const isStatic = !subItem.href;

                  return isStatic ? (
                    <div key={subItem.name} className="flex items-center space-x-4 p-3 rounded-md cursor-default hover:bg-white/10">
                      <div className="w-10 h-10 bg-green-100 text-green-600 rounded-lg flex items-center justify-center">
                        <Icon className="w-5 h-5" />
                      </div>
                      <span className={cn("text-sm font-medium dance-text text-white transition-colors" ,
                                              shouldUseDarkText ? 'text-black' : 'text-white'
                      )}>{subItem.name}</span>
                    </div>
                  ) : (
            ...
            <div key={link.name}>
              {link.items ? (
                <>
                  ...
                  >
                    {link.name}
                    <ChevronDown className={`transition-transform ${openMobileDropdown === link.name ? 'rotate-180' : ''}`} />
                  </button>
                  {openMobileDropdown === link.name && (
                    <div className="mt-2 ml-4 space-y-2">
                      {link.items.map(sub => {
                        ...
                        const Icon = sub.icon;
                        return isStatic ? (
                          <div key={sub.name} className="flex items-center text-base text-gray-800 cursor-default">
